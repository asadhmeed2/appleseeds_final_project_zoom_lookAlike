...
const Register = () => {
    ...
    const handleRegister =()=>{
        ...
        if((username === "")){
          ...
          let data = {userName:username,email:email, password: password,varifyPassword:varifyPassword,registerNumber:secritCode};
          // axios.post('http://localhost:4000/register',data).then(response =>{
          axios.post('https://asad-zoom-look-alike-server.herokuapp.com/register',data).then(response=>{
             navigate('/');
          }).catch((error)=>{
          console.log(error);
          })
    ...
    return (
                <div style={{ padding: 30 }} className="register">
                  <Paper >
                    ...
                    >
                      <Grid item xs={12}>
                        <TextField label="userName"  type="text" value={username} onFocus={(e)=>{usernameErrorRef.current.innerHTML=""}} onChange={(e)=>{setUsername(e.target.value)}} ></TextField>
                      </Grid>
                      <Grid item xs={12}>
                      ...
                      <Grid item xs={12}>
                        <TextField label="varify Password"  type={"password"} value={varifyPassword} onFocus={(e)=>{varifyPasswordErrorRef.current.innerHTML=""}} onChange={(e)=>{setVarifyPassword(e.target.value)}}></TextField>
                      </Grid>
                      <Grid item xs={12}>
                          <div className="error" ref={varifyPasswordErrorRef}></div>
                      </Grid>
                      <Grid item xs={12}>
                        <TextField label="secrit code"  type="text" value={secritCode} onFocus={(e)=>{secritCodeErrorRef.current.innerHTML=""}} onChange={(e)=>{setSecritCode(e.target.value)}} ></TextField>
                      </Grid>
                      <Grid item xs={12}>
